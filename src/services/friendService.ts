...
export const getFriends = async (userId: string) => {
  try {
    const response = await axios.get(`${API_URL}/friends/friends/${userId}`);
...
export const acceptFriendRequest = async (requestId: string) => {
  try {
    const response = await axios.post(`${API_URL}/friends/accept-request/${requestId}`);
    return response.data;
  } catch (error) {
...
export const rejectFriendRequest = async (requestId: string) => {
  try {
    const response = await axios.post(`${API_URL}/friends/reject-request/${requestId}`);
    return response.data;
  } catch (error) {
...
export const deleteFriendship = async (userId1: string, userId2: string) => {
  try {
    const response = await axios.delete(`${API_URL}/friends/friendship/${userId1}/${userId2}`);
    return response.data;
  } catch (error) {
...
export const getMutualFriends = async (userId1: string, userId2: string) => {
  try {
    const response = await axios.get(`${API_URL}/friends/mutual-friends/${userId1}/${userId2}`);
    return response.data;
  } catch (error) {
    return [];
  }
...
// Función para obtener el ranking de un amigo
export const getFriendRank = async (friendId: string) => {
  try {
    const response = await axios.get(`${API_URL}/friends/friend-rank/${friendId}`);
    return response.data.rankIndex;
  } catch (error) {
    return undefined;
  }
}; 